...
// given base config, e.g. runtime.NewDefaultConfig(). An app built on top of mailroom with settings of its own can
...
// loadConfig is LoadConfig with the args passed explicitly and the outcome returned, so that it can be tested, and
// used by commands with flags of their own which have to take those out of the command line first, see
// runtime.SplitArgs.
func loadConfig(cfg interface{ Parse() error }, args []string) error {
	loader := ezconf.NewLoader(cfg, "mailroom", "Mailroom - handler for RapidPro", []string{"mailroom.toml"})
	loader.SetArgs(args...)
