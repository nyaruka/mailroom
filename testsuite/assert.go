...
func AssertBatchTasks(t *testing.T, orgID models.OrgID, expected map[string]int, msgAndArgs ...any) {
	...
	tasks0, err := redis.Strings(rc.Do("LRANGE", fmt.Sprintf("{tasks:batch:%d}/0", orgID), 0, -1))
	require.NoError(t, err)

	tasks1, err := redis.Strings(rc.Do("LRANGE", fmt.Sprintf("{tasks:batch:%d}/1", orgID), 0, -1))
	require.NoError(t, err)
