...
func InsertChannel(t *testing.T, rt *runtime.Runtime, org *Org, typ models.ChannelType, name, address string, schemes []string, role string, config map[string]any) *Channel {
	...
	err := rt.DB.Get(&id,
		`INSERT INTO channels_channel(uuid, org_id, channel_type, name, address, schemes, role, config, last_seen, is_system, is_enabled, is_active, created_on, modified_on, created_by_id, modified_by_id)
		VALUES($1, $2, $3, $4, $5, $6, $7, $8, NOW(), FALSE, TRUE, TRUE, NOW(), NOW(), 1, 1) RETURNING id`, uuid, org.ID, typ, name, address, pq.Array(schemes), role, models.JSONB[map[string]any]{V: config},
	)
