...
func TestBroadcastsFromEvents(t *testing.T) {
	...
	for i, tc := range tcs {
		...
		err = tasks.Queue(ctx, rt, tc.queue, testdb.Org1.ID, &tasks.SendBroadcast{Broadcast: bcast}, false)
		assert.NoError(t, err)

		taskCounts := testsuite.FlushTasks(t, rt)

		// assert the tasks that were run
		assert.Equal(t, map[string]int{"send_broadcast": 1, "send_broadcast_batch": tc.expectedBatchCount}, taskCounts, "%d: unexpected tasks", i)
