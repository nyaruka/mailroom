...
func TestStartFlowBatchTask(t *testing.T) {
	...
	err = tasks.Queue(ctx, rc, rt.Queues.Throttled, testdb.Org1.ID, &starts.StartFlowBatchTask{FlowStartBatch: batch1}, false)
	assert.NoError(t, err)
	testsuite.FlushTasks(t, rt)

	assertdb.Query(t, rt.DB, `SELECT count(*) FROM flows_flowsession WHERE contact_id = ANY($1) 
		AND status = 'C' AND call_id IS NULL AND output IS NOT NULL`, pq.Array([]models.ContactID{testdb.Cathy.ID, testdb.Bob.ID})).
		Returns(2)
	...
	err = tasks.Queue(ctx, rc, rt.Queues.Throttled, testdb.Org1.ID, &starts.StartFlowBatchTask{FlowStartBatch: batch2}, false)
	assert.NoError(t, err)
	testsuite.FlushTasks(t, rt)
	...
	// start the first batch...
	err = tasks.Queue(ctx, rc, rt.Queues.Throttled, testdb.Org1.ID, &starts.StartFlowBatchTask{FlowStartBatch: start2Batch1}, false)
	assert.NoError(t, err)
	testsuite.FlushTasks(t, rt)
	...
	err = tasks.Queue(ctx, rc, rt.Queues.Throttled, testdb.Org1.ID, &starts.StartFlowBatchTask{FlowStartBatch: start2Batch2}, false)
	assert.NoError(t, err)
	testsuite.FlushTasks(t, rt)
...
func TestStartFlowBatchTaskNonPersistedStart(t *testing.T) {
	...
	// start the first batch...
	err := tasks.Queue(ctx, rc, rt.Queues.Throttled, testdb.Org1.ID, &starts.StartFlowBatchTask{FlowStartBatch: batch}, false)
	assert.NoError(t, err)
	testsuite.FlushTasks(t, rt)
