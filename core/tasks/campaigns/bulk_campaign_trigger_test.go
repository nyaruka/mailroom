...
func TestBulkCampaignTriggerUsingFireUUIDs(t *testing.T) {
	ctx, rt := testsuite.Runtime()
	...
	// create task for event #3 (Pick A Number, start mode SKIP)
	task := &campaigns.BulkCampaignTriggerTask{FireUUID: testdata.RemindersEvent3.FireUUID, ContactIDs: []models.ContactID{testdata.Bob.ID, testdata.Cathy.ID, testdata.Alexandria.ID}}
	...
	// create task for event #2 (single message, start mode PASSIVE)
	task = &campaigns.BulkCampaignTriggerTask{FireUUID: testdata.RemindersEvent2.FireUUID, ContactIDs: []models.ContactID{testdata.Bob.ID, testdata.Cathy.ID, testdata.Alexandria.ID}}
	err = task.Perform(ctx, rt, oa)
	assert.NoError(t, err)
	...
	// create task for event #1 (Favorites, start mode INTERRUPT)
	task = &campaigns.BulkCampaignTriggerTask{FireUUID: testdata.RemindersEvent1.FireUUID, ContactIDs: []models.ContactID{testdata.Bob.ID, testdata.Cathy.ID, testdata.Alexandria.ID}}
	err = task.Perform(ctx, rt, oa)
	assert.NoError(t, err)
	...
	// test task when campaign event has been updated)
	rt.DB.MustExec(`UPDATE campaigns_campaignevent SET fire_uuid = $2 WHERE id = $1`, testdata.RemindersEvent1.ID, uuids.NewV4())
	models.FlushCache()
	oa = testdata.Org1.Load(rt)
