...
func (t *MsgReceivedTask) Perform(ctx context.Context, rt *runtime.Runtime, oa *models.OrgAssets, mc *models.Contact) error {
	...
	if (trigger != nil && trigger.TriggerType() != models.CatchallTriggerType && (flow == nil || !flow.IgnoreTriggers())) ||
		...
		if flow != nil {
			...
			_, err = runner.StartFlow(ctx, rt, oa, flow, []*models.Contact{mc}, []flows.Trigger{trigger}, flow.FlowType().Interrupts(), models.NilStartID, models.NilCallID, sceneInit, nil)
			if err != nil {
	...
	if session != nil && flow != nil {
		...
		_, err = runner.ResumeFlow(ctx, rt, oa, session, mc, resume, sceneInit, nil)
		if err != nil {
