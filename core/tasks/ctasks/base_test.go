...
func TestTimedEvents(t *testing.T) {
	...
	for i, tc := range tcs {
		...
		if tc.eventType == ctasks.TypeMsgReceived {
			...
			ctask = &ctasks.WaitExpired{SessionUUID: sessionUUID, SprintUUID: taskSprintUUID}
		} else if tc.eventType == ctasks.TypeWaitTimeout {
			ctask = &ctasks.WaitTimeout{SessionUUID: sessionUUID, SprintUUID: taskSprintUUID}
		}

		err := tasks.QueueContact(ctx, rt, testdb.Org1.ID, testdb.Ann.ID, ctask)
