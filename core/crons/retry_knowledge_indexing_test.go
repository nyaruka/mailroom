...
func TestRetryKnowledgeIndexing(t *testing.T) {
	...
	k2 := testdb.InsertKnowledgeSource(t, rt, testdb.Org1, "78bee0eb-a3d1-4e2b-b91b-6ee1c2f1ab19", models.KnowledgeSourceTypeShortcuts, "Stuck", models.KnowledgeSourceStatusIndexing)
	rt.DB.MustExec(`UPDATE knowledge_knowledgesource SET modified_on = NOW() - INTERVAL '2 hours' WHERE id = $1`, k2.ID)
	...
	k3 := testdb.InsertKnowledgeSource(t, rt, testdb.Org2, "0e2e1c66-c221-4726-a08a-1a4bbabf05be", models.KnowledgeSourceTypeShortcuts, "Test Shortcuts", models.KnowledgeSourceStatusReady)
	rt.DB.MustExec(`UPDATE knowledge_knowledgesource SET last_indexed_on = NOW() WHERE id = $1`, k3.ID)
	...
	assertdb.Query(t, rt.DB, `SELECT status FROM knowledge_knowledgesource WHERE id = $1`, k2.ID).Returns("I")
	assertdb.Query(t, rt.DB, `SELECT status FROM knowledge_knowledgesource WHERE id = $1`, k3.ID).Returns("R")
