	...
	tcs := []handlers.TestCase{
		{
			...
			SQLAssertions: []handlers.SQLAssertion{
				...
				{
					// Verify the contact was removed from the smart group (age > 18) after age was set to 15
					SQL:   `select count(*) from contacts_contactgroup_contacts where contactgroup_id = $1 AND contact_id = $2`,
					Args:  []any{testdb.DoctorsGroup.ID, testdb.Cathy.ID},
					Count: 0,
				},
			},
		},
