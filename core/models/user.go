...
// explicit memberships, excluding anyone who is in one of the org's admin groups, unioned with the members of those
// groups who always have the administrator role and no team
...
LEFT JOIN LATERAL (SELECT id, uuid, name FROM tickets_team WHERE tickets_team.id = m.team_id) AS team_struct ON True
            WHERE m.org_id = $1 AND u.is_active = TRUE
              AND NOT EXISTS (SELECT 1 FROM orgs_org_admin_groups ag INNER JOIN users_user_groups ug ON ug.group_id = ag.group_id WHERE ag.org_id = $1 AND ug.user_id = u.id)
        UNION ALL
           SELECT u.id, u.uuid, u.email, u.first_name, u.last_name, 'A' AS role_code, NULL::json AS team
             FROM users_user u
            WHERE u.is_active = TRUE
              AND EXISTS (SELECT 1 FROM orgs_org_admin_groups ag INNER JOIN users_user_groups ug ON ug.group_id = ag.group_id WHERE ag.org_id = $1 AND ug.user_id = u.id)
         ORDER BY email ASC
) r;`

// loadUsers loads all the users for the passed in org, including members of its admin groups
