...
func handleDelete(ctx context.Context, rt *runtime.Runtime, r *deleteRequest) (any, int, error) {
	...
	rows, err := rt.DB.QueryContext(ctx, `SELECT uuid, contact_id FROM msgs_msg WHERE org_id = $1 AND uuid = ANY($2) AND direction = 'I' AND visibility NOT IN ('D', 'X') ORDER BY id`, r.OrgID, pq.Array(r.MsgUUIDs))
	if err != nil {
